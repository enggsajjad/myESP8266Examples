...
void loop (void)
{
  ...
  if((SPSR & (1 << SPIF)) != 0)
  {
    spiHandler();
    //dat = SPDR;
    //Serial.print(dat,DEC);Serial.println (" ");
    //SPDR = dat+1;
   }
...
void spiHandler()
{
  ...
  case 5:
    ...
    marker++;
    executeCommand();
    SPDR = 23;//resultBuffer.resultChar[0];    
    break;    
  case 6:
    marker++;
    SPDR = 45;//resultBuffer.resultChar[1]; 
    break;   
  case 7:
    dat = SPDR;
    marker=0;
...
void executeCommand(void)
{
 ...
 Serial.println (receiveBuffer[0],DEC);
 Serial.println (receiveBuffer[1],DEC);
 Serial.println (receiveBuffer[2],DEC);
 Serial.println (receiveBuffer[3],DEC);
 Serial.println (receiveBuffer[4],DEC);
 Serial.println ("-------------------");
