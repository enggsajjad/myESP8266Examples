...
Ticker timer5(printCountUS, 125, 0, MICROS_MICROS);
...
void printCountUS() {
  buf0[counterUS++] = map(analogRead(A0),0,1023,0,255);
  
  if (counterUS == (1024*40)) {
    timer5.stop();
    //counterUS = 0;
    }
  }
...
void loop()
{
  ...
  if(recording)
  {
    ...
    //delay(2000);
    //timer5.start();
    while((cnt125us<40960)&&(recDone == false))
    {
      int old=micros();
      //buf0[cnt125us++] = map(analogRead(mic),0,1023,0,255);
      unsigned char mValue = analogRead(mic);
      buf0[cnt125us++] = (mValue>255)?255:mValue;
